/********************************************************************
    created:   2012/12/2
    filename:  read_consume_reward_conf.cpp
    author:    xhm 
    depend:    libxml2.lib 
*********************************************************************/
...
extern Recharge_reward_time_T consume_reward_time_conf;//时间配置文件；
extern Recharge_reward_detail_T consume_reward_detail_conf;//奖励配置文件；

int read_consume_reward_conf(char *fileName)
{
	xmlDocPtr doc;              
	xmlNodePtr curNode;
	char szDocName[50]={0};
	...
	char* tempInfo=NULL;
	int tempSize=0;
	int memSize=sizeof(consume_reward_detail_conf.reward_goods_id[0]);
	consume_reward_time_conf.init();//时间配置初始化；
	consume_reward_detail_conf.init();//奖励配置初始化；
	
	doc=xmlReadFile(szDocName,"UTF-8",XML_PARSE_RECOVER);
	if(NULL==doc)
	{
		cout<<"[XHM WARNING] xmlReadFile ERROR(read_consume_reward_conf.cpp)!"<<endl;
		return -1;
	}
	...
	if(NULL==curNode)
	{
		cout<<"[XHM WARNING] xmlDocGetRootElement ERROR(read_consume_reward_conf.cpp)!"<<endl;	
		xmlFreeDoc(doc);
		return -3;
	}
	//xml的相关变量
		...
		if(!xmlStrcmp(curNode->name,(const xmlChar *)"date"))
		{
			...
			{	
				if(!xmlStrcmp(attrPtr->name,(const xmlChar*)"year"))
				{
					...
					consume_reward_time_conf.year=atoi(const_cast<char*>(reinterpret_cast<const char*>(year)));
					xmlFree(year);
				}else if(!xmlStrcmp(attrPtr->name,(const xmlChar*)"month"))
				{
					xmlChar *month=xmlGetProp(propNodePtr_time,(const xmlChar*)"month");
					consume_reward_time_conf.month=atoi(const_cast<char *>(reinterpret_cast<const char *>(month)));
					xmlFree(month);
				}else if(!xmlStrcmp(attrPtr->name,(const xmlChar*)"dayStart"))
				{
					xmlChar *dayStart=xmlGetProp(propNodePtr_time,(const xmlChar*)"dayStart");
					consume_reward_time_conf.start_day=atoi(const_cast<char *>(reinterpret_cast<const char *>(dayStart)));
					xmlFree(dayStart);
				}else if(!xmlStrcmp(attrPtr->name,(const xmlChar*)"dayEnd"))
				{
					xmlChar *dayEnd=xmlGetProp(propNodePtr_time,(const xmlChar*)"dayEnd");
					consume_reward_time_conf.end_day=atoi(const_cast<char *>(reinterpret_cast<const char *>(dayEnd)));
					xmlFree(dayEnd);
				}else if(!xmlStrcmp(attrPtr->name,(const xmlChar*)"levelSize"))
				{
					...
					consume_reward_detail_conf.levelcounts=atoi(const_cast<char *>(reinterpret_cast<const char *>(levelSize)));
					xmlFree(levelSize);
				}
		...
		}else if(!xmlStrcmp(curNode->name,(const xmlChar *)"reward"))
		{
			...
			{	
				...
				{	
					...
					if(consume_reward_detail_conf.levelcounts < index)
					{
						cout<<"[XHM MSG] read_consume_reward_conf.cpp read config ERROR,index:"<<index<<endl;
						return -1;
					}
					xmlFree(level);
				...
				{	
					...
					memcpy(consume_reward_detail_conf.reward_goods_id[index],tempInfo,memSize);
					xmlFree(goodsIdA);
				...
				{	
					...
					memcpy(consume_reward_detail_conf.reward_goods_id2[index],tempInfo,memSize);
					xmlFree(goodsIdB);
				...
				{	
					...
					consume_reward_detail_conf.levelSize[index]=tempSize;
					xmlFree(rechargeNum);
				...
				{	
					...
					consume_reward_detail_conf.reward_goods_num[index]=tempSize;
					xmlFree(sizeA);
				...
				{	
					...
					consume_reward_detail_conf.reward_goods_num2[index]=tempSize;
					xmlFree(sizeB);
				...
				{	
					...
					consume_reward_detail_conf.bind_gold[index]=tempSize;
					xmlFree(bind_gold);
				}		
