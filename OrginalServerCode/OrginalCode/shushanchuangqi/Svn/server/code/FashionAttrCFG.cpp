...
CFashionAttrLoader::~CFashionAttrLoader()
{
	ClearData();
}

void CFashionAttrLoader::ClearData()
{
	for(size_t i=0; i<_vecData.size();++i)
	{
		SFashionAttrCFG *p = _vecData[i];
		delete p;
	}
	_vecData.clear();
}

bool CFashionAttrLoader::LoadData(const std::string& strPath)
{
	ClearData();
	TiXmlDocument doc;
	...
	{
		SFashionAttrCFG* pData = new SFashionAttrCFG;
		...
		if(pstr_material != NULL)
			pData->_material = (UINT32)atoi(pstr_material);
		else
			pData->_material = 0;
		const char* pstr_EffectIdle1 = pDataElem->Attribute("EffectIdle1");
		if(pstr_EffectIdle1 != NULL)
			pData->_EffectIdle1 = pstr_EffectIdle1;
		else
			pData->_EffectIdle1 = "";
		const char* pstr_EffectIdle2 = pDataElem->Attribute("EffectIdle2");
		if(pstr_EffectIdle2 != NULL)
			pData->_EffectIdle2 = pstr_EffectIdle2;
		else
			pData->_EffectIdle2 = "";
		const char* pstr_EffectSkin = pDataElem->Attribute("EffectSkin");
		if(pstr_EffectSkin != NULL)
			pData->_EffectSkin = pstr_EffectSkin;
		else
			pData->_EffectSkin = "";
		const char* pstr_EffectSkinWaiting = pDataElem->Attribute("EffectSkinWaiting");
		if(pstr_EffectSkinWaiting != NULL)
			pData->_EffectSkinWaiting = pstr_EffectSkinWaiting;
		else
			pData->_EffectSkinWaiting = "";
