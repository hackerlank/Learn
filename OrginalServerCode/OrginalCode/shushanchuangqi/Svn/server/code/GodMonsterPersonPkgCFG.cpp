...
// 文件名：     SGodMonsterPersonPkgCFG.cpp
...
CGodMonsterPersonPkgLoader::~CGodMonsterPersonPkgLoader()
{
	ClearData();
}

void CGodMonsterPersonPkgLoader::ClearData()
{
	for(size_t i=0; i<_vecData.size();++i)
	{
		SGodMonsterPersonPkgCFG *p = _vecData[i];
		delete p;
	}
	_vecData.clear();
}

bool CGodMonsterPersonPkgLoader::LoadData(const std::string& strPath)
{
	ClearData();
	TiXmlDocument doc;
	...
	{
		SGodMonsterPersonPkgCFG* pData = new SGodMonsterPersonPkgCFG;
