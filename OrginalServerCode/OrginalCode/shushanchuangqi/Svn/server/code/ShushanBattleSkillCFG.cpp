...
// 文件名：     SShushanBattleSkillCFG.cpp
...
CShushanBattleSkillLoader::~CShushanBattleSkillLoader()
{
	ClearData();
}

void CShushanBattleSkillLoader::ClearData()
{
	for(size_t i=0; i<_vecData.size();++i)
	{
		SShushanBattleSkillCFG *p = _vecData[i];
		delete p;
	}
	_vecData.clear();
}

bool CShushanBattleSkillLoader::LoadData(const std::string& strPath)
{
	ClearData();
	TiXmlDocument doc;
	...
	{
		SShushanBattleSkillCFG* pData = new SShushanBattleSkillCFG;
		...
		if(pstr_TaskId != NULL)
			pData->_TaskId = (UINT8)atoi(pstr_TaskId);
		else
			pData->_TaskId = 0;
		const char* pstr_CostType = pDataElem->Attribute("CostType");
		if(pstr_CostType != NULL)
			pData->_CostType = (UINT8)atoi(pstr_CostType);
		else
			pData->_CostType = 0;
		...
		const char* pstr_EFIGHTATTR_COUTEREXTRAADD = pDataElem->Attribute("EFIGHTATTR_COUTEREXTRAADD");
		if(pstr_EFIGHTATTR_COUTEREXTRAADD != NULL)
			pData->_EFIGHTATTR_COUTEREXTRAADD = (UINT32)atoi(pstr_EFIGHTATTR_COUTEREXTRAADD);
		else
			pData->_EFIGHTATTR_COUTEREXTRAADD = 0;
		const char* pstr_EFIGHTATTR_COUTEREXTRARDC = pDataElem->Attribute("EFIGHTATTR_COUTEREXTRARDC");
		if(pstr_EFIGHTATTR_COUTEREXTRARDC != NULL)
			pData->_EFIGHTATTR_COUTEREXTRARDC = (UINT32)atoi(pstr_EFIGHTATTR_COUTEREXTRARDC);
		else
			pData->_EFIGHTATTR_COUTEREXTRARDC = 0;
