...
CCollectionLevelLoader::~CCollectionLevelLoader()
{
	ClearData();
}

void CCollectionLevelLoader::ClearData()
{
	for(size_t i=0; i<_vecData.size();++i)
	{
		SCollectionLevelCFG *p = _vecData[i];
		delete p;
	}
	_vecData.clear();
}

bool CCollectionLevelLoader::LoadData(const std::string& strPath)
{
	ClearData();
	TiXmlDocument doc;
	...
	{
		SCollectionLevelCFG* pData = new SCollectionLevelCFG;
