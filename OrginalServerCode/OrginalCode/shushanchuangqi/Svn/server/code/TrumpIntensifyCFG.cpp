...
CTrumpIntensifyLoader::~CTrumpIntensifyLoader()
{
	ClearData();
}

void CTrumpIntensifyLoader::ClearData()
{
	for(size_t i=0; i<_vecData.size();++i)
	{
		STrumpIntensifyCFG *p = _vecData[i];
		delete p;
	}
	_vecData.clear();
}

bool CTrumpIntensifyLoader::LoadData(const std::string& strPath)
{
	ClearData();
	TiXmlDocument doc;
	...
	{
		STrumpIntensifyCFG* pData = new STrumpIntensifyCFG;
		const char* pstr_ItemAttrID = pDataElem->Attribute("ItemAttrID");
		if(pstr_ItemAttrID != NULL)
			pData->_ItemAttrID = (UINT16)atoi(pstr_ItemAttrID);
		else
			pData->_ItemAttrID = 0;
		const char* pstr_Rank = pDataElem->Attribute("Rank");
		if(pstr_Rank != NULL)
			pData->_Rank = (UINT8)atoi(pstr_Rank);
		else
			pData->_Rank = 0;
		...
		if(pstr_SkillLevel != NULL)
			pData->_SkillLevel = (UINT8)atoi(pstr_SkillLevel);
		else
			pData->_SkillLevel = 0;
		...
		if(pstr_Type != NULL)
			...
			pData->_Type = 0;
		const char* pstr_ExpNeed = pDataElem->Attribute("ExpNeed");
		if(pstr_ExpNeed != NULL)
			pData->_ExpNeed = (UINT32)atoi(pstr_ExpNeed);
		else
			pData->_ExpNeed = 0;
		const char* pstr_ProbNeed = pDataElem->Attribute("ProbNeed");
		if(pstr_ProbNeed != NULL)
			pData->_ProbNeed = (UINT32)atoi(pstr_ProbNeed);
		else
			pData->_ProbNeed = 0;
		...
		const char* pstr_EvolveItemCount = pDataElem->Attribute("EvolveItemCount");
		if(pstr_EvolveItemCount != NULL)
			pData->_EvolveItemCount = (UINT32)atoi(pstr_EvolveItemCount);
		else
			pData->_EvolveItemCount = 0;
