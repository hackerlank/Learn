...
// 文件名：     SFighterTaoismCFG.cpp
// Copyright(c) 2012-2100 上海恺英网络科技有限公司，All rights reserved.
// ******************************************************************************************************/


#include "FighterTaoismCFG.h"
#include "tinyxml.h"

CFighterTaoismLoader::~CFighterTaoismLoader()
{
	ClearData();
}

void CFighterTaoismLoader::ClearData()
{
	for(size_t i=0; i<_vecData.size();++i)
	{
		SFighterTaoismCFG *p = _vecData[i];
		delete p;
	}
	_vecData.clear();
}

bool CFighterTaoismLoader::LoadData(const std::string& strPath)
{
	ClearData();
	TiXmlDocument doc;
	...
	{
		SFighterTaoismCFG* pData = new SFighterTaoismCFG;
		...
		if(pstr_Level != NULL)
			...
			pData->_Level = 0;
		const char* pstr_NeedTaoism = pDataElem->Attribute("NeedTaoism");
		if(pstr_NeedTaoism != NULL)
			pData->_NeedTaoism = (UINT32)atoi(pstr_NeedTaoism);
		else
			pData->_NeedTaoism = 0;
