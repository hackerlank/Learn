...
// 文件名：     SDaily_yamentaskColorCFG.cpp
...
CDaily_yamentaskColorLoader::~CDaily_yamentaskColorLoader()
{
	ClearData();
}

void CDaily_yamentaskColorLoader::ClearData()
{
	for(size_t i=0; i<_vecData.size();++i)
	{
		SDaily_yamentaskColorCFG *p = _vecData[i];
		delete p;
	}
	_vecData.clear();
}

bool CDaily_yamentaskColorLoader::LoadData(const std::string& strPath)
{
	ClearData();
	TiXmlDocument doc;
	...
	{
		SDaily_yamentaskColorCFG* pData = new SDaily_yamentaskColorCFG;
