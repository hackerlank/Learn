...
CStoveAttrLoader::~CStoveAttrLoader()
{
	ClearData();
}

void CStoveAttrLoader::ClearData()
{
	for(size_t i=0; i<_vecData.size();++i)
	{
		SStoveAttrCFG *p = _vecData[i];
		delete p;
	}
	_vecData.clear();
}

bool CStoveAttrLoader::LoadData(const std::string& strPath)
{
	ClearData();
	TiXmlDocument doc;
	...
	{
		SStoveAttrCFG* pData = new SStoveAttrCFG;
		const char* pstr_StoveID = pDataElem->Attribute("StoveID");
		if(pstr_StoveID != NULL)
			pData->_StoveID = (UINT8)atoi(pstr_StoveID);
		else
			pData->_StoveID = 0;
		...
		if(pstr_Level != NULL)
			...
			pData->_Level = 0;
		const char* pstr_SpeedAddition = pDataElem->Attribute("SpeedAddition");
		if(pstr_SpeedAddition != NULL)
			pData->_SpeedAddition = (UINT32)atoi(pstr_SpeedAddition);
		else
			pData->_SpeedAddition = 0;
