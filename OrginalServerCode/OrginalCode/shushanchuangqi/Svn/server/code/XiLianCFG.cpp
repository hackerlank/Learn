...
CXiLianLoader::~CXiLianLoader()
{
	ClearData();
}

void CXiLianLoader::ClearData()
{
	for(size_t i=0; i<_vecData.size();++i)
	{
		SXiLianCFG *p = _vecData[i];
		delete p;
	}
	_vecData.clear();
}

bool CXiLianLoader::LoadData(const std::string& strPath)
{
	ClearData();
	TiXmlDocument doc;
	...
	{
		SXiLianCFG* pData = new SXiLianCFG;
		const char* pstr_ID = pDataElem->Attribute("ID");
		if(pstr_ID != NULL)
			pData->_ID = (UINT16)atoi(pstr_ID);
		else
			pData->_ID = 0;
		const char* pstr_Shuffer1 = pDataElem->Attribute("Shuffer1");
		if(pstr_Shuffer1 != NULL)
			pData->_Shuffer1 = pstr_Shuffer1;
		else
			pData->_Shuffer1 = "";
		const char* pstr_Shuffer2 = pDataElem->Attribute("Shuffer2");
		if(pstr_Shuffer2 != NULL)
			pData->_Shuffer2 = pstr_Shuffer2;
		else
			pData->_Shuffer2 = "";
		const char* pstr_Shuffer3 = pDataElem->Attribute("Shuffer3");
		if(pstr_Shuffer3 != NULL)
			pData->_Shuffer3 = pstr_Shuffer3;
		else
			pData->_Shuffer3 = "";
		const char* pstr_Attrnum = pDataElem->Attribute("Attrnum");
		if(pstr_Attrnum != NULL)
			pData->_Attrnum = (UINT8)atoi(pstr_Attrnum);
		else
			pData->_Attrnum = 0;
		const char* pstr_Color = pDataElem->Attribute("Color");
		if(pstr_Color != NULL)
			pData->_Color = pstr_Color;
		else
			pData->_Color = "";
