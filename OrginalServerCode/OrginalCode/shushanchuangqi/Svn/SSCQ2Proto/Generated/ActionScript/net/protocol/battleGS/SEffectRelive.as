...
package net.protocol.battleGS
{
	 ...
	 * 重生
	 */
	public final class SEffectRelive extends SEffectBase
	{
		...
		public override function toXML(name_:String = null):XML
		{
			var topXml:XML
			if(name_ != null && name_ != "")
				topXml = <SEffectRelive _name_={name_} byGroupID={byGroupID} bySide={bySide} byPos={byPos} wFighterID={wFighterID} dwHpMax={dwHpMax}
				 dwHp={dwHp} dwAura={dwAura} dwAuraMax={dwAuraMax}/>;
			else
				topXml = <SEffectRelive byGroupID={byGroupID} bySide={bySide} byPos={byPos} wFighterID={wFighterID} dwHpMax={dwHpMax}
				 dwHp={dwHp} dwAura={dwAura} dwAuraMax={dwAuraMax}/>;
		...
		public static function fromByteArray(bytes:ByteArray):SEffectRelive
		{
			var s_:SEffectRelive = new SEffectRelive();
		...
		public static function vectorFromByteArray(bytes:ByteArray):Vector.<SEffectRelive>
		{
			var length:uint = BytesUtil.readVectorLength(bytes);
			var vec:Vector.<SEffectRelive> = new Vector.<SEffectRelive>();
			for(var i:uint = 0; i < length; ++i)
			{
				var s:SEffectRelive = SEffectRelive.fromByteArray(bytes);
				vec.push(s);
			}
			return vec;
		}
		...
		public static function vectorToByteArray(vec:Vector.<SEffectRelive>, bytes:ByteArray):void
		{
			BytesUtil.writeVectorLength(bytes, vec.length);
			for each(var s:SEffectRelive in vec)
				s.toByteArray(bytes);
		}
	}
}
