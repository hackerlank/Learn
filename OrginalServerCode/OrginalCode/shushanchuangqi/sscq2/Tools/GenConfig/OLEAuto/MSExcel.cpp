...
HRESULT CMSExcel::GetExcelValue(LPCTSTR szCell, CString &sValue, LPCTSTR szType)
{
	if(!m_pEApp) return E_FAIL;
	if(!m_pActiveBook) return E_FAIL;

	static set<CString> setValidType;
	if(setValidType.empty())
	{
		setValidType.insert(L"INT8");
		setValidType.insert(L"UINT8");
		setValidType.insert(L"INT16");
		setValidType.insert(L"UINT16");
		setValidType.insert(L"INT32");
		setValidType.insert(L"UINT32");
		setValidType.insert(L"INT64");
		setValidType.insert(L"UINT64");
		setValidType.insert(L"FLOAT");
		setValidType.insert(L"DOUBLE");
		setValidType.insert(L"BOOL");
		setValidType.insert(L"STRING");
	}

	CString strType = szType;
	strType.Trim(L" \n\t");
	if(setValidType.find(strType) == setValidType.end())
	{
		CString msg;
...
/*
		VARIANT itemn;
		if(CString(szType) == L"STRING")
		{
			//string
//			itemn.vt = VT_BSTR;
//			itemn.bstrVal = ::SysAllocString(L"111111111111111111111111111111");
		}
		else if(CString(szType) == L"INT32"
			|| CString(szType) == L"UINT32")
		{
			itemn.vt = VT_I4;
			itemn.lVal = 1;
		}
		else if(CString(szType) == L"BOOL")
		{
			itemn.vt = VT_I4;
			itemn.lVal = 1;
		}
		else if(CString(szType) == L"DOUBLE")
		{
			itemn.vt = VT_R8;
			itemn.dblVal = 1;
		}
		else if(CString(szType) == L"INT64"
			|| CString(szType) == L"UINT64")
		{
			itemn.vt = VT_I4;
			itemn.lVal = 1;
		}
		else if(CString(szType) == L"INT16"
			|| CString(szType) == L"UINT16")
		{
			itemn.vt = VT_I4;
			itemn.lVal = 1;
		}
		else if(CString(szType) == L"INT8"
			|| CString(szType) == L"UINT8")
		{
			itemn.vt = VT_I4;
			itemn.lVal = 1;
		}
		else if(CString(szType) == L"FLOAT")
		{
			itemn.vt = VT_R8;
			itemn.lVal = 1;
		}
...
*/
		...
		if(result.vt == VT_R8)
		{
			...
			if(strType == L"INT8")
			{
				retValue.Format(L"%d", (INT8)result.dblVal);
			}
			else if(strType == L"UINT8")
			{
				retValue.Format(L"%u", (UINT8)result.dblVal);
			}
			else if(strType == L"INT16")
			{
				retValue.Format(L"%d", (INT16)result.dblVal);
			}
			else if(strType == L"UINT16")
			{
				retValue.Format(L"%u", (UINT16)result.dblVal);
			}
			else if(strType == L"INT32")
			{
				retValue.Format(L"%d", (INT32)result.dblVal);
			}
			else if(strType == L"UINT32")
			{
				retValue.Format(L"%u", (UINT32)result.dblVal);
			}
			else if(strType == L"INT64")
			{
				retValue.Format(L"%lld", (INT64)result.dblVal);
			}
			else if(strType == L"UINT64")
			{
				retValue.Format(L"%llu", (UINT64)result.dblVal);
			}
			else if(strType == L"STRING")
			{
				bool bisF = false;
				int ivalue = (int)result.dblVal;
				if(result.dblVal-ivalue != 0)
					bisF = true;
				if(!bisF)
					retValue.Format(L"%d", ivalue);
				else
					retValue.Format(L"%d", result.dblVal);
			}
			else if(strType == L"FLOAT")
			{
				retValue.Format(L"%f", (float)result.dblVal);
			}
			else if(strType == L"BOOL")
			{
				retValue.Format(L"%u", result.dblVal>0?1:0);
			}
			else if(strType == L"DOUBLE")
			{
				retValue.Format(L"%f", result.dblVal);
			}
			else
			{
				AfxMessageBox(L"定义类型有错误！");
		...
		{
			sValue.Format(L"%u", result.dblVal>0?1:0);
		}
	}
	pRange->Release();
	pSheet->Release();
