...
bool BasicExcelCell::Get(int& val) const
{
	if (type_ == INT) {
		...
		std::wstring wstr(wstr_.data(), wstr_.size());
		val = std::stoi(wstr);
		return true;
...
bool BasicExcelCell::Get(long long& val) const
{
    if (type_ == INT) {
        val = ival_;
        return true;
...
// Get a big integer value.
// Returns 0 if cell does not contain a big integer.
long long BasicExcelCell::GetBigInteger() const
{
    long long val;
...
const wchar_t* BasicExcelCell::GetWString() const
{
	...
	if (type_ == WSTRING) {
		...
		if (GetString())
		{
			const std::vector<wchar_t>* pvec = &wstr_;
			wstring wstr = ToWString(GetString(), CP_UTF8);
			const_cast<std::vector<wchar_t>*>(pvec)->resize(wstr.size()+1, 0);
			const_cast<std::vector<wchar_t>*>(pvec)->assign(wstr.begin(), wstr.end());
			return &*(wstr_.begin());
		}
	}
