...
CLevelActMgr g_LevelActMgr;

bool CLevelActMgr::CheckConds(UINT32 wConds)
{
...
void CLevelActMgr::TakePrize(UINT64 &qwPlayerID,UINT32 wCond)
{
    UINT16 wActID = GetActID();
...
void CLevelActMgr::OnEnd()
{
   ...
   for(auto &oIt : wConds)
   {
        UINT32 dwCount = CUserMgr::GetCount(NGame2CenterComm::eActLevel,oIt); 
...
void CLevelActMgr::OnEvent(SActEventData& oSActEventData)
{
    UINT16 wActID = GetActID();
    UINT32 wCond = oSActEventData.dwCond;
    
    NWonderActivityProt::SWonderPrize2DB sPrize2DB;
    sPrize2DB.wActID = wActID;
    sPrize2DB.wActType = 1 //条件完成
    sPrize2DB.wCond = wCond;
    sPrize2DB.qwRoleID = 1;
    sPrize2DB.byStatus = 1;
    g_Center2DBCommC.Send_UpdatePrizeStatus(sPrize2DB,eOpAdd);
    
    SWonderPrizeKey oKey(1,wActID,wCond);
...
void CLevelActMgr::GetRoleStateByActID(UINT64 qwRoleID,UINT16 wActID,TVecRoleStatus& vecRet)
{
    UINT32 dwMaxCount;
    TVecUINT32 vecConds;
    GetConds(vecConds);
...
UINT8 CLevelActMgr::GetPrizeStateForRole(UINT64 qwRoleID,UINT16 wActID,UINT32 wCond)
{
...
UINT32 CLevelActMgr::GetCurRoleCount(UINT16 wActID,UINT32 wCond)
{
    UINT32 dwCount = 0;
    ...
    dwCount = CUserMgr::GetCount(NGame2CenterComm::eActLevel,wCond);
    return dwCount;
}
