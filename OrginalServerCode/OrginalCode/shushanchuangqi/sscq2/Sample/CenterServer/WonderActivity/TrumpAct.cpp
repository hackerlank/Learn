...
bool CTrumpAct::CheckConds(UINT32 wConds)
{
    ...
    UINT32 wCount = GetMaxRoleCount(wConds);
    UINT32 wRoles = GetCurRoleCount(wActID,wConds);
    if(wRoles >= wCount)
        return true;
    return false;
}

void CTrumpAct::TakePrize(UINT64 &qwPlayerID,UINT32 wCond)
{
    UINT16 wActID = GetActID();
...
void CTrumpAct::OnEnd()
{
   ...
   for(auto &oIt : wConds)
   {
        UINT32 dwCount = CUserMgr::GetCount(NGame2CenterComm::eActTrump,oIt); 
...
void CTrumpAct::GetRoleStateByActID(UINT64 qwRoleID,UINT16 wActID,TVecRoleStatus& vecRet)
{
    UINT32 dwMaxCount;
    TVecUINT32 vecConds;
    GetConds(vecConds);
...
UINT8 CTrumpAct::GetPrizeStateForRole(UINT64 qwRoleID,UINT16 wActID,UINT32 wCond)
{
...
UINT32 CTrumpAct::GetCurRoleCount(UINT16 wActID,UINT32 wCond)
{
    UINT32 dwCount = 0;
    ...
    dwCount = CUserMgr::GetCount(NGame2CenterComm::eActTrump,wCond);
    return dwCount;
}
