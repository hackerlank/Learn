...
CSkillAttrLoader::~CSkillAttrLoader()
{
	ClearData();
}

void CSkillAttrLoader::ClearData()
{
	for(size_t i=0; i<_vecData.size();++i)
	{
		SSkillAttrCFG *p = _vecData[i];
		delete p;
	}
	_vecData.clear();
}

bool CSkillAttrLoader::LoadData(const std::string& strPath)
{
	ClearData();
	TiXmlDocument doc;
	...
	{
		SSkillAttrCFG* pData = new SSkillAttrCFG;
