...
CFighterLvAttrLoader::~CFighterLvAttrLoader()
{
	ClearData();
}

void CFighterLvAttrLoader::ClearData()
{
	for(size_t i=0; i<_vecData.size();++i)
	{
		SFighterLvAttrCFG *p = _vecData[i];
		delete p;
	}
	_vecData.clear();
}

bool CFighterLvAttrLoader::LoadData(const std::string& strPath)
{
	ClearData();
	TiXmlDocument doc;
	...
	{
		SFighterLvAttrCFG* pData = new SFighterLvAttrCFG;
		...
		if(pstr_Job != NULL)
			...
			pData->_Job = 0;
		const char* pstr_StarType = pDataElem->Attribute("StarType");
		if(pstr_StarType != NULL)
			pData->_StarType = (UINT8)atoi(pstr_StarType);
		else
			pData->_StarType = 0;
		...
		if(pstr_EFIGHTATTR_MAXHP != NULL)
			pData->_EFIGHTATTR_MAXHP = (FLOAT)atof(pstr_EFIGHTATTR_MAXHP);
		else
			pData->_EFIGHTATTR_MAXHP = 0.0f;
		...
		if(pstr_EFIGHTATTR_INITAURA != NULL)
			pData->_EFIGHTATTR_INITAURA = (FLOAT)atof(pstr_EFIGHTATTR_INITAURA);
		else
			pData->_EFIGHTATTR_INITAURA = 0.0f;
		const char* pstr_EFIGHTATTR_MAXAURA = pDataElem->Attribute("EFIGHTATTR_MAXAURA");
		if(pstr_EFIGHTATTR_MAXAURA != NULL)
			pData->_EFIGHTATTR_MAXAURA = (FLOAT)atof(pstr_EFIGHTATTR_MAXAURA);
		else
			pData->_EFIGHTATTR_MAXAURA = 0.0f;
