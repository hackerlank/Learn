...
// 文件名：     SFighterCombinationEffectCFG.cpp
...
CFighterCombinationEffectLoader::~CFighterCombinationEffectLoader()
{
	ClearData();
}

void CFighterCombinationEffectLoader::ClearData()
{
	for(size_t i=0; i<_vecData.size();++i)
	{
		SFighterCombinationEffectCFG *p = _vecData[i];
		delete p;
	}
	_vecData.clear();
}

bool CFighterCombinationEffectLoader::LoadData(const std::string& strPath)
{
	ClearData();
	TiXmlDocument doc;
	...
	{
		SFighterCombinationEffectCFG* pData = new SFighterCombinationEffectCFG;
