...
CFighterCuiTiLoader::~CFighterCuiTiLoader()
{
	ClearData();
}

void CFighterCuiTiLoader::ClearData()
{
	for(size_t i=0; i<_vecData.size();++i)
	{
		SFighterCuiTiCFG *p = _vecData[i];
		delete p;
	}
	_vecData.clear();
}

bool CFighterCuiTiLoader::LoadData(const std::string& strPath)
{
	ClearData();
	TiXmlDocument doc;
	...
	{
		SFighterCuiTiCFG* pData = new SFighterCuiTiCFG;
		...
		if(pstr_SilverNeed != NULL)
			...
			pData->_SilverNeed = 0;
		const char* pstr_LimitLevel = pDataElem->Attribute("LimitLevel");
		if(pstr_LimitLevel != NULL)
			pData->_LimitLevel = (UINT8)atoi(pstr_LimitLevel);
		else
			pData->_LimitLevel = 0;
