...
CPetLevelLoader::~CPetLevelLoader()
{
	ClearData();
}

void CPetLevelLoader::ClearData()
{
	for(size_t i=0; i<_vecData.size();++i)
	{
		SPetLevelCFG *p = _vecData[i];
		delete p;
	}
	_vecData.clear();
}

bool CPetLevelLoader::LoadData(const std::string& strPath)
{
	ClearData();
	TiXmlDocument doc;
	...
	{
		SPetLevelCFG* pData = new SPetLevelCFG;
		...
		if(pstr_Level != NULL)
			...
			pData->_Level = 0;
		const char* pstr_LevelUpExp = pDataElem->Attribute("LevelUpExp");
		if(pstr_LevelUpExp != NULL)
			pData->_LevelUpExp = (UINT32)atoi(pstr_LevelUpExp);
		else
			pData->_LevelUpExp = 0;
		const char* pstr_TrainItem = pDataElem->Attribute("TrainItem");
		if(pstr_TrainItem != NULL)
			pData->_TrainItem = (UINT32)atoi(pstr_TrainItem);
		else
			pData->_TrainItem = 0;
		const char* pstr_TrainItemNum = pDataElem->Attribute("TrainItemNum");
		if(pstr_TrainItemNum != NULL)
			pData->_TrainItemNum = (UINT8)atoi(pstr_TrainItemNum);
		else
			pData->_TrainItemNum = 0;
		const char* pstr_TrainExp = pDataElem->Attribute("TrainExp");
		if(pstr_TrainExp != NULL)
			pData->_TrainExp = (UINT32)atoi(pstr_TrainExp);
		else
			pData->_TrainExp = 0;
