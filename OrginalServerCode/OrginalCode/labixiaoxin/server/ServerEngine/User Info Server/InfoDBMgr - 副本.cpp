...
//character_tmp_tbl [character_id,team_info,rank,pvp_season,pvp_team_info]
//config_tbl [current_pvp_season]
//pvp_season_tbl0 [character_id,score,all,win,season_id,update_time]
//team_rank_tbl[character_id,point,update_time,team_info]
...
/*
void InfoDBMgr::update_team_rank(ACE_UINT32 unCharacterID, ACE_UINT32 unPoint, PlayerTeamInfo *teamInfo)
{
////team_rank_tbl[character_id,point,update_time,team_info]
m_prep_stmt = m_dbConn->prepareStatement("select character_id from team_rank_tbl where character_id=?");
if(m_prep_stmt == NULL)
{
return;
}
...
if(insertFlag)
{
m_prep_stmt = m_dbConn->prepareStatement("insert into team_rank_tbl(point,team_info,update_time,character_id) values(?,?,?,?)");
}
else
{
m_prep_stmt = m_dbConn->prepareStatement("update team_rank_tbl set point=?,team_info=?,update_time=? where character_id=?");
}
...
DBDataBuf data((char *)teamInfo, sizeof(PlayerTeamInfo));
std::istream is(&data);

m_prep_stmt->setInt(1, unPoint);
m_prep_stmt->setBlob(2, &is);
m_prep_stmt->setInt(3, GameUtils::get_utc());
m_prep_stmt->setInt(4, unCharacterID);
m_db_res = m_prep_stmt->executeQuery();
destroy_db_res();
}
*/
