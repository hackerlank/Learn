 ...
 * \return 验证登陆指令是否成功
 */
bool GatewayTask::loginGateway(const ProtoMsgData::ReqLoginGateway *message)
{
    do
    {
        ...
        if (!handle) 
        {   
            Fir::logger->debug("[登录网关]:请求登录失败(memdb连接失败,%s)",message->login().account().c_str());
            break;
        }   

        // 应该在need_gate_id上登录
        DWORD need_gate_id = handle->getInt("gatewaylogin",message->login().registerkey().c_str(), "gate_id");
        // 当前gateway是
        DWORD cur_gate_id = GatewayService::getMe().getServerID();
        if(need_gate_id != cur_gate_id)
        {   
            Fir::logger->debug("[登录网关]:请求登录失败(gateid不对,%u,%u,%s)",cur_gate_id,need_gate_id,message->login().account().c_str());
            break;
        }   
        handle->setInt("gatewaylogin",message->login().registerkey().c_str(),"state",GATEWAY_USER_LOGINSESSION_STATE_REG);
        strncpy(m_accountInfo.account,message->login().account().c_str(),sizeof(m_accountInfo.account));
        if(message->login().registertype() == ProtoMsgData::RT_Phone)
        {
            strncpy(m_accountInfo.phoneNum,message->login().registerkey().c_str(),sizeof(m_accountInfo.phoneNum));
        }
        else
        {
            strncpy(m_accountInfo.email,message->login().registerkey().c_str(),sizeof(m_accountInfo.email));
        }
        this->changeLoginState();
        this->charid = handle->getInt("charbaseinfo",message->login().registerkey().c_str(),"charid");
        if(this->charid == 0)
        {
            Fir::logger->debug("[登录网关]:请求登录成功(等待创建角色,%lu,%s)",charid,m_accountInfo.account);
            m_verify = true;
            break;
        }
        handle = zMemDBPool::getMe().getMemDBHandle(this->charid % MAX_MEM_DB+1);
        if (!handle) 
        {   
            Fir::logger->debug("[登录网关]:请求登录失败(memdb连接失败,%s)",m_accountInfo.account);
            break;
        } 

        CharBase charbase;
        if (handle->getBin("charbase",this->charid,"charbase",(char*)&charbase) == 0)
        {
            Fir::logger->debug("[登录网关]:请求登录失败(rides获取不到记录,%lu,%s)",charid,m_accountInfo.account);
            break;
        }
        Fir::logger->debug("[登录网关]:请求登录成功(rides获取到记录,%lu,%s)",charid,m_accountInfo.account);
        m_verify = true;
        break;
    }while(false);
    return m_verify;
...
int GatewayTask::verifyConn()
{
    ...
    if (retcode > 0)
    {
        while(true)
        {
            ...
            {
                Fir::logger->debug("[登录网关] (消息非法,%s)",m_accountInfo.account);
                return -1;
            }
            if(msgParseProto(pstrCmd,nCmdLen) && m_verify)
            {
                Fir::logger->debug("[登录网关]:请求登录成功(验证%s)",m_accountInfo.account);
                return 1;
            }
            Fir::logger->debug("[登录网关]:请求登录失败(消息错误,%s)",m_accountInfo.account);
            return -1;
        }
    }
    else
...
void GatewayTask::Terminate(const TerminateMethod method)
{
    ...
    if (m_pUser)
    {
        Fir::logger->trace("[GS],charid=%lu,注销 ip=%s",m_pUser->charid,getIP());
        if(!is_terminate)
        {
            m_pUser->unreg();
            m_pUser->final();
...
{
    GateUser* pUser = GateUserManager::getMe().getUserCharid(charid);
    if(pUser)
    {
        return ;
    }
    pUser = new GateUser(charid, this);
    if(!pUser)
    {
        return ;
    }
    if (pUser)
    {
        Fir::logger->debug("[登陆网关]:登录游戏成功(创建GateUser,%lu,%s)",charid,m_accountInfo.account);
        //发送游戏时间，开始计时
        ProtoMsgData::AckSystemTime message;
        message.set_time(Fir::qwGameTime);
        if(!qwGameTime)
        {
        ...
        if (pUser->charid == 0)
        {
            Fir::logger->debug("[登录网关]:登录游戏成功(创建GateUser且等待创建角色,%lu,,%s)",charid,m_accountInfo.account);
            pUser->createState();

            //没有角色，系统先帮他创建一个吧
            ProtoMsgData::ReqCreateRole createMessage;
            Fir::logger->debug("[登录网关]:登录游戏成功(系统创建角色,%lu,%s)",charid,m_accountInfo.account);
        ...
        else
        {
            if(pUser->beginSelect())
            {
                ...
                {
                    Fir::logger->debug("[登录网关]:登录游戏失败(添加到GateUserManager,%lu,%s)",charid,m_accountInfo.account);
                    return ;
                }
                //通知登陆服务器，网关连接数
            ...
            {
                Fir::logger->debug("[客户端登录_3]:登录游戏失败(beginSelect失败,%lu,%s)",charid,m_accountInfo.account);
                SAFE_DELETE(pUser);
                m_pUser = NULL;
        ...
        case USERGAMETIME_TIMER_USERCMD_PARA:
            {
                ...
                {   
                    if(this->m_pUser)
                    {
                        Fir::logger->trace("客户端游戏时间太快，使用了加速器，需要断开连接%u,%lu",ptNullCmd->dwTimestamp, rev->qwGameTime - (qwGameTime + (GatewayTimeTick::currentTime.sec() - GameTimeSyn.sec())));
                    }
                    Fir::logger->debug("游戏时间:(%lu,%lu)",rev->qwGameTime, qwGameTime + (GatewayTimeTick::currentTime.sec() - GameTimeSyn.sec()));
                    TerminateWait();
                }
...
void GatewayTask::initProtoDispatch()
{
    ...
    s_protoNameToServerTypeMap[HelloKittyMsgData::ReqAllFunctionBuild::descriptor()] = SCENESSERVER;
    ...
    s_protoNameToServerTypeMap[HelloKittyMsgData::ReqGetMessageList::descriptor()] = SCENESSERVER;
    s_protoNameToServerTypeMap[HelloKittyMsgData::ReqServerNotice::descriptor()] = SCENESSERVER;
    s_protoNameToServerTypeMap[HelloKittyMsgData::ReqOwnerDelMessage::descriptor()] = SCENESSERVER;
