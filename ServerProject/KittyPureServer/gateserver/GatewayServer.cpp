...
bool GatewayService::init()
{
    ...
    {
        Fir::logger->debug("[网关服] 初始化网络服务器失败");
        return false;
    }
    ...
    {
        Fir::logger->debug("[网关服] 初始化任务连接池失败");
        return false;
    }
        ...
        case PARA_SUPER2GATE_KICK_OUT_USER:
            {
                ...
                const char *key = strlen(rev->accountInfo.email) ? rev->accountInfo.email : rev->accountInfo.phoneNum;
                QWORD charID = handle->getInt("charbaseinfo",key,"charid");
...
int main(int argc, char **argv)
{
    ...
    {
        Fir::logger->debug("[网关服] 读取config文件失败");
        return EXIT_FAILURE;
    }
