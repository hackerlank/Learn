...
bool zMemDB::getSortSet(const char* table,const char* myset, std::map<QWORD,DWORD>& valueMap,const DWORD begin,const DWORD end)
{
    ...
    if (getHandle())
    {
        if (myset)
        {
            reply = (redisReply*)redisCommand(c,"zrange %s %d %d withscores",myset,begin,end);
        }
...
bool zMemDB::setSortSet(const char* table, QWORD id, const char* myset, const char* value)
{
    ...
    if (getHandle())
    {
        if (myset)
        {
            reply = (redisReply*)redisCommand(c, "ZADD %s %s %lu",myset,value,id);
        }
        PROCESS_REPLY_ERROR	
    }
...
bool zMemDB::setSortSet(const char* table, QWORD id, const char* myset, const QWORD value)
{
    ...
    if (getHandle())
    {
        if (myset)
        {
            reply = (redisReply*)redisCommand(c, "ZADD %s %lu %lu",myset,value,id);
        }
        PROCESS_REPLY_ERROR	
    }
