		...
		case PARA_SESSION_NEWSESSION:
			{
				...
				if (!handle) 
				{
                    Fir::logger->error("[客户端登录_2]:客户端请求网关失败(memdb连接失败,%s)",ptCmd->session.accountInfo.account);
					return false;
				}
                const char *key = strlen(ptCmd->session.accountInfo.phoneNum) ? ptCmd->session.accountInfo.phoneNum : ptCmd->session.accountInfo.email;
                int state = 0;
                state = handle->getInt("gatewaylogin" ,key,"state");
                if(state == 0)
                {
                    bool flag = handle->setInt("gatewaylogin",key,"gate_id",ptCmd->session.wdGatewayID);
					//玩家不在线，更新会话信息
                    if(!flag)
                    {
                        Fir::logger->error("[客户端登录_2]:客户端请求网关失败(redis设置网关失败,%s,%u)",ptCmd->session.accountInfo.account,ptCmd->session.wdGatewayID);
                        return false;
                    }
                    std::string ret;
                    encodeMessage(flNullCmd,nCmdLen,ret);
                    this->sendCmd(ret.c_str(),ret.size());
                    DWORD nowGateID = handle->getInt("gatewaylogin",key,"gate_id");
                    Fir::logger->error("[客户端登录_2]:客户端请求网关成功(更新网关,%s,%u)",ptCmd->session.accountInfo.account,nowGateID);
					return true;
				}
				else
				{
                    DWORD old_gate_id = handle->getInt("gatewaylogin",key,"gate_id");
                    bool flag = handle->setInt("gatewaylogin",key,"gate_id",ptCmd->session.wdGatewayID);
                    
					// 把同账号踢下线
					ptCmd->session.login_ret = 1; 
                    CMD::SUPER::t_Super2GateKickOutUser send;
                    send.accountInfo = ptCmd->session.accountInfo;
                    std::string ret;
                    ...
                    if(flag)
                    {
                        Fir::logger->error("[客户端登录_2]:客户端请求网关成功(剔除且更新网关,%s)",ptCmd->session.accountInfo.account);
                    }
					return true;
				}
				return true;
