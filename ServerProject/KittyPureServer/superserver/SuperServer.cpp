...
bool SuperService::init()
{
    ...
    if (NULL == dbConnPool || !dbConnPool->putURL(0, Fir::global["mysql"].c_str(), false) || !metaData || !metaData->init(Fir::global["mysql"]))
    {
        Fir::logger->debug("[管理服] 连接数据库失败");
        return false;
    }

    if (!zMemDBPool::getMe().init() || (NULL == zMemDBPool::getMe().getMemDBHandle()))
    {
        Fir::logger->debug("[管理服] 连接内存数据失败");
        return false;
    }
    ...
    if (!getServerInfo())
    {
        Fir::logger->debug("[管理服] 加载服务器组失败");
        return false;
    }

    if (!FLClientManager::getMe().init())
    {
        Fir::logger->debug("[管理服] 连接登录服务器失败");
        return false;
    }
    ...
    {
        Fir::logger->debug("[管理服] 初始化任务连接池失败");
        return false;
    }
    ...
    {
        Fir::logger->debug("[管理服] 初始化网络服务器失败");
        return false;
    }

    if(!zMNetService::bind("SuperServer端口",wdPort))
    {
        Fir::logger->debug("[管理服] 监听端口失败");
        return false;
    }

    if(!zMNetService::bind("http端口", http_port))
    {
        Fir::logger->debug("[管理服] 监听PHP端口失败");
        return false;
    }
...
int main(int argc, char **argv)
{
    ...
    {
        Fir::logger->debug("[管理服] 读取config文件失败");
        return EXIT_FAILURE;
    }
