...
bool ServerTask::msgParseGyList(const CMD::FL::t_GYList_FL *ptCmd, const DWORD nCmdLen)
{
	using namespace CMD::FL;
    GYList gy;
    Fir::logger->debug("gateway更新状态:zoneid=%u,serverid=%u,ip=%s,port=%u,rolenum=%u,state=%u,nettype=%u", gameZone.zone, ptCmd->wdServerID, ptCmd->pstrIP, ptCmd->wdPort, ptCmd->wdNumOnline, ptCmd->state, ptCmd->wdNetType);
    ...
    return GYListManager::getMe().put(gy);
}
