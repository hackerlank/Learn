...
bool FLService::init()
{
	...
	{
        Fir::logger->debug("[登录服务器] 初始化连接数据库错误");
		return false;
	}
	...
	{
		Fir::logger->debug("[登录服务器] 邀请码管理器失败");
		return false;
	}

	if (!zMNetService::init())
	{
        Fir::logger->debug("[登录服务器] 初始化网络服务器失败");
		return false;
	}
	...
	if (NULL == loginTaskPool || !loginTaskPool->init())
    {
        Fir::logger->debug("[登录服务器] 初始化任务连接池失败");
		return false;
    }
    ...
    {
        Fir::logger->debug("[登录服务器] 初始化线程池失败");
		return false;
    }
    ...
    {
        Fir::logger->debug("[登录服务器] 初始化php任务连接池失败");
		return false;
    }

    Fir::XMLParser cmdXml;
    cmdXml.load_from_file("cmdxml/cmdXml.xml");
    if(!CmdFunManager::getMe().loadCmd(cmdXml))
    {
        Fir::logger->debug("[登录服务器] 初始化指令集失败");
        return false;
    }
...
int main(int argc, char **argv)
{
    ...
    {
        Fir::logger->debug("[登录服务器] 读取config文件失败");
		return EXIT_FAILURE;
    }
