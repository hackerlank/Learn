...
class RecordService : public zSubNetService, public Singleton<RecordService>
{
	...
	public:
        ...
        bool loadCashMaxID();
        //加载寿司历史记录
        bool loadSushiData(const ProtoMsgData::RecordSerialize &recordBinary);
        //保存寿司历史记录
        bool saveSuShiData(ProtoMsgData::RecordSerialize &recordBinary);
        DWORD getMaxsqlLen();
    public:
    ...
    private:
        ...
        //反序列化二进制数据
        void setupBinaryArchive(const ProtoMsgData::RecordSerialize& recordBinary);
        //序列化二进制数据
